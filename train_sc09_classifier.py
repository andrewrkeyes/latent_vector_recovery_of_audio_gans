...
for epoch in range(starting_epoch, 100):
  ...
  dataset = tf.data.Dataset.from_generator(valid_generator, output_types=(tf.int64, tf.float32)).shuffle(500).batch(64)
  train_loss.reset_states()
  train_acc.reset_states()
  for label, audio in dataset:
    test_step(audio, label)

    writer.add_scalar("train_loss", test_loss.result().numpy(), n_iter_test)
    writer.add_scalar("train_acc", test_acc.result().numpy(), n_iter_test)
    train_loss.reset_states()
    train_acc.reset_states()
    n_iter_test += 1
